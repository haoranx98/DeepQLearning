...
class ReplayMemory(object):
    ...
    def push(self, *args):
        ...
        self.memory[self.position] = Transition(
            *args)  # 参数前面加上* 号 ，意味着参数的个数不止一个，另外带一个星号（*）参数的函数传入的参数存储为一个元组（tuple），带两个（*）号则是表示字典（dict）
...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, outputs):
        ...
        self.conv1 = nn.Conv2d(3, 16, kernel_size=5,
                               stride=2)  # in_channels：输入的通道数目,out_channels： 输出的通道数目,kernel_size：卷积核的大小，类型为int 或者元组，当卷积是方形的时候，只需要一个整数边长即可，卷积不是方形，要输入一个元组表示 高和宽。stride： 卷积每次滑动的步长为多少
        self.bn1 = nn.BatchNorm2d(
            16)  # 在卷积神经网络的卷积层之后总会添加BatchNorm2d进行数据的归一化处理,num_features：一般输入参数为batch_sizenum_featuresheight*width，即为其中特征的数量
...
resize = T.Compose([T.ToPILImage(),  # convert a tensor to PIL image
                    T.Resize(40, interpolation= Resampling.BICUBIC),  # 图像变换
...
plt.imshow(get_screen().cpu().squeeze(0).permute(1, 2, 0).numpy(),
           interpolation='none')  # plt.imshow()函数负责对图像进行处理，并显示其格式，但是不能显示。其后跟着plt.show（）才能显示出来
...
def optimize_model():
    ...
    loss = F.smooth_l1_loss(state_action_values, expected_state_action_values.unsqueeze(
        1))  # torch.unsqueeze()这个函数主要是对数据维度进行扩充。需要通过dim指定位置，给指定位置加上维数为1的维度。
    # 优化模型
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        _, reward, done, _ , _ = env.step(action.item())  # 将选择的action输入给env，env 按照这个动作走一步进入下一个状态
